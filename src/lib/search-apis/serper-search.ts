...
function extractInfoFromSerperResult(result: SerperSearchResult, searchName: string): SearchResult {
...
async function performSerperSearch(apiKey: string, query: string, name: string): Promise<SearchResult[]> {
  ...
  return data.organic.map(result => extractInfoFromSerperResult(result, name));
}

export async function searchWithSerper(name: string): Promise<SearchResult[]> {
